...
"List of Cancer Types in COSMIC 3.0 Dataset"
...
"List of Exogenous Signatures in COSMIC 3.0 Dataset"
signatures = {"SBS1": "Age",
              "SBS4": "Tobacco",
              "SBS7b": "UV",
              "SBS11": "Temozolomide",
...
def somatic_sim(cancer_type, reading_frame, std_outlier, number_of_lineages, simulation_type, sequence_abs_path, slice_start, slice_end,power=1, syn_rate=1, non_syn_rate=1):
    ...
    if cancer_type not in cancer_type_list + list(sbs_num_data.columns[3:]):
        print("cancer_type argument must be in " + str(cancer_type_list + list(sbs_num_data.columns[3:])))
        sys.exit()
    ...
    sample_seq = seq_slice(sequence_abs_path, slice_start=None, slice_end=None)
    
    #Synonymous codons 
    syn_codon = syn_codon_dict(codon_dict)
    
    if cancer_type in list(sbs_num_data.columns[3:]):
        ...
        for seq_len in range(0, len(sample_seq)-3):
            if 'N' in sample_seq[seq_len:seq_len+3]:
                pass
    
            else:
                sample_index_dict[str(sample_seq[seq_len:seq_len+3])].append(seq_len)
        ...
        signature_df[cancer_type] = sbs_prop_data[cancer_type]/sbs_prop_data[cancer_type].sum()
        
        #Sample mutations
        mutation_array = np.random.multinomial(n_mutations, list(signature_df[cancer_type]))
        ...
        print("Simulation " + str(number_of_lineages) + " set up.")
        ...
        print("Simulation " + str(number_of_lineages) + " complete.")
        #Make directories
        try:
            os.mkdir("Sequence/" + cancer_type)
        except OSError:
            print ("Creation of the directory failed.")
        
        try:
            os.mkdir("Metadata/" + cancer_type)
        except OSError:
            print ("Creation of the directory failed.")
            
    
        #Write mutated sequence to fasta file
        sample_sequence_file = "Sequence/" + cancer_type + "/" + cancer_type + '_Sequence_' + str(number_of_lineages) + '.fasta'
        ...
        print("Simulation " + str(number_of_lineages) + " output successful.")
...
class New_wind_open:
    def __init__(self , root , txt ):
        ...
        self.new_wind.title('SomaticSiMu_EM')
        ...
        Label( self.frame4 , text = 'SomaticSiMu_EM' , bg = 'DarkBlue',  fg = 'white' , font = ( 'verdana' , 20 ) ).pack()
...
def open_img(root , name):
    ...
    Label(frame6 , text = 'SomaticSiMu_EM' ,bg = 'Darkblue' , fg = 'White' , font = ('verdana' , 14) ).pack()
...
def gui():
    ...
    Label( frame3 , text = 'SomaticSiMu_EM' , bg = 'DarkBlue',  fg = 'white' , font = ( 'verdana' , 30 ) ).pack()
    ...
    n = StringVar() 
    Button(screen1 , text = 'Simulation'  , bg= 'White' , fg = 'Black' , font = ('Helvitka' , 10 , 'bold' ) , command = lambda : open_img( screen1 , abs_path('cancer_type.png', "File" ) )).place(x = 80 , y = 140)  
    Cancer_type = ttk.Combobox( screen1 , height = 12 , width = 14 ,textvariable = n) 
    Cancer_type['values'] = (cancer_type_list + list(sbs_num_data.columns[3:])) 
    ...
    Button( screen1 , text = 'Stage' , bg= 'White' , fg = 'Black' , font = ('Helvitka' , 10 , 'bold' ) ,  command = lambda : open_img( screen1 , abs_path('simulation_type.png', "File" ) ) ).place(x = 80 , y = 380)
    n3=StringVar()
...
def gui2():
    ...
    Label( frame2 , text = 'SomaticSiMu_EM' , bg = 'DarkBlue' , fg = 'white' , font = ( 'verdana' , 30 ) ).pack()

    n3 = StringVar() 
    Button(screen2 , text = 'Simulation' , bg= 'White' , fg = 'Black'  , font = ('Helvitka' , 12 , 'bold' ) ,  command = lambda : open_img( screen2 , abs_path('viz_cancer_type.png', "File" ) )).place(x = 60 , y = 80)  
    Cancer_type2 = ttk.Combobox( screen2 , height = 12 , width = 14 ,textvariable = n3) 
    Cancer_type2['values'] = (cancer_type_list + list(sbs_num_data.columns[3:]) )
    ...
    Mut_type['values'] = ( ['SBS', 'DBS' , 'Insertion' , 'Deletion', 'Mutation Burden', "SBS_Signature"] )  
...
def mut_catalog(cancer_type, simulation_type, gen_start, gen_end, mut_type):
    try:
        ...
        if mut_type.lower() == "sbs_signature":
            
            graph_data = pd.DataFrame(index=(range(96)), columns=range(gen_start, gen_end+1))
            
            for gen in range(gen_start, gen_end+1):
                df_read_in = pd.read_csv(abs_path(cancer_type + '_Sequence_' + str(gen)+ '_sbs_freq_table.csv', 'File'))
                graph_data[gen] = df_read_in['Count']
            graph_data['mean'] = graph_data.mean(axis=1)
                    
            graph_data['SubType'] = df_read_in['SubType']
            graph_data['Type'] = df_read_in['Type']
            
            graph_data['final'] = graph_data['mean'].div(graph_data['mean'].sum())
            ...
            p1 = ax.bar(range(0,16), graph_data.iloc[0:16, -1].multiply(100), bottom=0, color = color_list[0])
            p2 = ax.bar(range(16,32), graph_data.iloc[16:32, -1].multiply(100), bottom=0, color = color_list[1])
            p3 = ax.bar(range(32,48), graph_data.iloc[32:48, -1].multiply(100), bottom=0, color = color_list[2])
            p4 = ax.bar(range(48,64), graph_data.iloc[48:64, -1].multiply(100), bottom=0, color = color_list[3])
            p5 = ax.bar(range(64,80), graph_data.iloc[64:80, -1].multiply(100), bottom=0, color = color_list[4])
            p6 = ax.bar(range(80,96), graph_data.iloc[80:96, -1].multiply(100), bottom=0, color = color_list[5])
...
class Main_window:
    def __init__(self):
        ...
        self.main.title( 'SomaticSiMu_EM' )
        self.frame = Frame( self.main , height=200, width=200 )
        self.frame.config(background="Dark Blue")
        self.frame.pack(fill='both')
        Label( self.frame , text = 'SomaticSiMu_EM' , bg = 'Dark Blue' , fg = 'White' , font = ( 'verdana' , 30 ) ).pack()
...
if __name__ ==  '__main__':
    ...
    print("SomaticSiMu_EM is set up.")
